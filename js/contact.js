...
PortfolioUtils.initPage('../', () => {
  ...
  function validateForm() {
    ...
    const v1 = validate('cf-name',    'err-name',    name.length >= 2,        'Name must be at least 2 characters.');
    const v2 = validate('cf-email',   'err-email',   emailRx.test(email),     'Please enter a valid email address.');
    const v3 = validate('cf-subject', 'err-subject', subject.length >= 3,     'Subject must be at least 3 characters.');
    const v4 = validate('cf-message', 'err-message', message.length >= 10,    'Message must be at least 10 characters.');
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      if (typeof emailjs !== 'undefined' && EMAILJS_PUBLIC_KEY !== 'YOUR_PUBLIC_KEY') {
        ...
        window.open(`mailto:rajveersinghshekhawat3234@gmail.com?subject=${subject}&body=${body}`);
        showToast('Opening email client... (Set up EmailJS for direct delivery)', 'info');
      }
    } catch (err) {
      console.error('EmailJS error:', err);
      showToast('Failed to send. Please email directly: rajveersinghshekhawat3234@gmail.com', 'error');
    }
